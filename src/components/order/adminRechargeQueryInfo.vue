      ...
      <el-form :inline="true" :model="formInline" class="demo-form-inline">
        ...
        <el-form-item label="">
          <el-select size="small" v-model="formInline.orderStatus" placeholder="请选择" style="width: 180px;">
            ...
            <el-option value="充值成功">充值成功</el-option>
            <el-option value="待处理">待处理</el-option>
            <el-option value="充值失败">充值失败</el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="">
    ...
    <div class="tableD">
      <el-table :data="tableData" :header-cell-style="{ background: 'rgba(250, 250, 250, 1)' }" style="width: 100%;font-size:13px;">
        ...
        <el-table-column label="订单状态" width="120">
          <template slot-scope='scope'>
            <div v-if="scope.row.orderStatus=='充值成功'" style="color: green;width: 80px;height: 25px;line-height: 25px;text-align: center;border-radius: 5px">
            ...
            <div v-if="scope.row.orderStatus=='待处理'" style="color: orange;width: 80px;height: 25px;line-height: 25px;text-align: center;border-radius: 5px">
              {{scope.row.orderStatus}}</div>
            <div v-if="scope.row.orderStatus=='充值失败'" style="color: red;width: 80px;height: 25px;line-height: 25px;text-align: center;border-radius: 5px">
              {{scope.row.orderStatus}}</div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="200">
          <template slot-scope="scope">
            <el-button-group>
              ...
              <el-button type="success" @click="notifyClick(scope.row)" icon="el-icon-check" size="small" :disabled="scope.row.orderStatus=='充值成功' || scope.row.orderStatus=='充值失败'">充值审核
              </el-button>
            </el-button-group>
          </template>
        </el-table-column>
    ...
    <el-dialog class="push" :visible.sync="checkDialog" width="550px">
      ...
      <el-form :model="checkForm" status-icon ref="ruleForm1" label-width="150px" class="demo-ruleForm1">
        ...
        <el-form-item label="状态:" style="text-align:left;margin-bottom:20px;">
          <el-radio v-model="checkForm.flag" label="充值成功" style="margin-top:14px;">充值成功</el-radio>
          <br>
          <el-radio v-model="checkForm.flag" label="充值失败" style="margin-top:14px;">充值失败</el-radio>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    nextSubmit() {
      ...
      API.phLogin("/czListS.do", data).then((res) => {
        if (res.data.code == "0000") {
