<template>
  <div class="userInfoQuery">
    <div class="textAL queryD">
      <el-form v-if="userPush" inline status-icon ref="ruleForm1" label-width="130px" class="demo-ruleForm1">
        <div>
          <el-form-item label="币种:" :rules="[{required:true}]">
            <span style="display:inline-block;width:300px">USDT</span>
          </el-form-item>
          <el-form-item label="币种数量:" :rules="[{required:true}]">
            <el-input type="number" style="width: 300px" v-model="formChange.uNum" placeholder="请输入币种数量"></el-input>
          </el-form-item>
          <el-form-item label="币种地址:" :rules="[{required:true}]">
            <el-input type="text" style="width: 300px" v-model="formChange.uAddr" placeholder="请输入币种地址"></el-input>
          </el-form-item>
          <el-form-item label="提现备注:">
            <el-input type="textarea" style="width: 300px;height: 50px" :maxlength="128" v-model="formChange.orderRsv" placeholder="请输入提现备注"></el-input>
          </el-form-item>
          <el-form-item label="" style="display:flex;">
            <div style="margin-left:53px;">
              <span style="color:red">
                <span>注:可提现账户余额&nbsp;</span>
                <span style="font-weight:bold">{{Number(userAmt).toFixed(2)}}</span>
                <span>（U）</span>
              </span>
            </div>
          </el-form-item>
        </div>
        <div style="text-align:center;margin-top:20px;">
          <el-button type="primary" size="small" @click="submitForm()" :loading="tXLoading">
            提交
          </el-button>
          <el-button size="small" @click="_resetRorm">
            重置
          </el-button>
          <el-button size="small" @click="userPush = false">
            返回
          </el-button>
        </div>
      </el-form>
      <el-form :inline="true" :model="formInline" class="demo-form-inline">
        <el-form-item label="">
          <el-date-picker size="small" format="yyyy-MM-dd" v-model="formInline.tDate" type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="">
          <el-select size="small" v-model="formInline.orderStatus" placeholder="请选择" style="width: 180px">
            <el-option value="">默认</el-option>
            <el-option value="提现成功">提现成功</el-option>
            <el-option value="待处理">待处理</el-option>
            <el-option value="提现失败">提现失败</el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="">
          <el-input size="small" v-model="formInline.orderNo" placeholder="请输入订单号" style="width: 180px"></el-input>
        </el-form-item>
        <el-form-item>
          <el-button size="small" type="primary" icon="el-icon-search" @click="onSubmit">查询
          </el-button>
        </el-form-item>
        <div style="margin-bottom:10px;">
          <el-button size="small" type="primary" icon="el-icon-plus" style="background-color: #1ec695;" @click="txClick">申请提现
          </el-button>
        </div>
      </el-form>
    </div>
    <div class="tableD">
      <el-table :data="tableData" :header-cell-style="{ background: 'rgba(250, 250, 250, 1)' }" style="width: 100%;font-size:13px;">
        <el-table-column fixed type="index" width="50" label="序号"></el-table-column>
        <el-table-column prop="orderNo" label="订单号" width="200"></el-table-column>
        <el-table-column prop="curcd" label="币种" width="120"></el-table-column>
        <el-table-column label="币种地址(U)" width="300">
          <template slot-scope='scope'>
            <div>数量：{{scope.row.uNum}} U</div>
            <div>地址：
              <span :style="{fontSize:scope.row.uAddr.length>20?'10px':'14px'}">{{scope.row.uAddr}}</span>
            </div>
          </template>
        </el-table-column>
        <el-table-column label="订单状态" width="120">
          <template slot-scope='scope'>
            <div v-if="scope.row.orderStatus=='提现成功'" style="color: green;width: 80px;height: 25px;line-height: 25px;text-align: center;border-radius: 5px">
              {{scope.row.orderStatus}}</div>
            <div v-if="scope.row.orderStatus=='待处理'" style="color: orange;width: 80px;height: 25px;line-height: 25px;text-align: center;border-radius: 5px">
              {{scope.row.orderStatus}}</div>
            <div v-if="scope.row.orderStatus=='提现失败'" style="color: red;width: 80px;height: 25px;line-height: 25px;text-align: center;border-radius: 5px">
              {{scope.row.orderStatus}}</div>
          </template>
        </el-table-column>
        <el-table-column label="时间" width="200">
          <template slot-scope='scope'>
            <div style="font-size: 12px">创建时间:{{scope.row.crtTime}}</div>
            <div style="font-size: 12px">完成时间:{{scope.row.upTime}}</div>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="220">
          <template slot-scope="scope">
            <el-button-group>
              <el-button icon="el-icon-info" size="small" style="background-color: #78909C;color:white;" @click="userinfo(scope.row.orderNo)">详情</el-button>
            </el-button-group>
          </template>
        </el-table-column>
      </el-table>
      <div class="tableMsg">
        <el-pagination layout="prev, pager, next" @current-change="currentC" :page-size="1" :total="allPage">
        </el-pagination>
      </div>
    </div>
    <!--详情-->
    <el-dialog class="instAbout" :visible.sync="instAboutBtn">
      <div slot="title" class="dialog-footer">
        <span>详情信息</span>
      </div>
      <el-form label-position="left" inline class="demo-table-expand">
        <template>
          <el-form-item label="订单号:">
            <span>{{instAboutList.orderNo}}&nbsp;</span>
          </el-form-item>
          <el-form-item label="订单金额(CNY):">
            <span>{{instAboutList.orderAmt}}&nbsp;</span>
          </el-form-item>
          <el-form-item label="手续费(USDT):">
            <span>{{instAboutList.orderFee}} U&nbsp;</span>
          </el-form-item>
          <el-form-item label="数量(USDT):">
            <span>{{instAboutList.uNum}} U&nbsp;</span>
          </el-form-item>
          <el-form-item label="转账地址(USDT):">
            <span :style="{fontSize:instAboutList.uAddr && instAboutList.uAddr.length>20?'12px':'14px'}">{{instAboutList.uAddr}}&nbsp;</span>
          </el-form-item>
          <el-form-item label="订单类型:">
            <span>{{instAboutList.orderType}}&nbsp;</span>
          </el-form-item>
          <el-form-item label="订单状态:">
            <span>{{instAboutList.orderStatus}}&nbsp;</span>
          </el-form-item>
          <el-form-item label="订单备注:">
            <div style="width: 600px!important;color:red;">{{instAboutList.orderRsv}}&nbsp;</div>
          </el-form-item>
          <el-form-item label="审核备注:">
            <div style="width: 600px!important;color:red;">{{instAboutList.chkRsv}}&nbsp;</div>
          </el-form-item>
          <el-form-item label="下游订单号:">
            <span>{{instAboutList.mchtOrderNo}}&nbsp;</span>
          </el-form-item>
          <el-form-item label="回调地址:">
            <span>{{instAboutList.mchtBackUrl}}&nbsp;</span>
          </el-form-item>
          <el-form-item label="回调状态:">
            <span>{{instAboutList.mchtBackSta}}&nbsp;</span>
          </el-form-item>
          <el-form-item label="操作员ID:">
            <span>{{instAboutList.operId}}&nbsp;</span>
          </el-form-item>
          <el-form-item label="操作员名称:">
            <span>{{instAboutList.operName}}&nbsp;</span>
          </el-form-item>
          <el-form-item label="创建时间:">
            <span>{{instAboutList.crtTime}}&nbsp;</span>
          </el-form-item>
          <el-form-item label="更新时间:">
            <span>{{instAboutList.upTime}}&nbsp;</span>
          </el-form-item>
          <el-form-item label>
            <span></span>
          </el-form-item>
        </template>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" style="background-color: #1ec695;border:1px solid #1ec695;" @click="instAboutBtn = false">确 定</el-button>
      </div>
    </el-dialog>
    <!--提现申请-->
    <!-- <el-dialog class="push" :visible.sync="userPush">
      <div slot="title" class="dialog-footer">
        <span>提现申请</span>
      </div>
      <el-form :model="formChange" status-icon ref="ruleForm1" label-width="130px" class="demo-ruleForm1">
        <el-form-item label="币种:" :rules="[{required:true}]">
          <span>USDT</span>
        </el-form-item>
        <el-form-item label="币种数量:" :rules="[{required:true}]">
          <el-input type="number" style="width: 330px" v-model="formChange.uNum" placeholder="请输入币种数量"></el-input>
        </el-form-item>
        <el-form-item label="币种地址:" :rules="[{required:true}]">
          <el-input type="text" style="width: 330px" v-model="formChange.uAddr" placeholder="请输入币种地址"></el-input>
        </el-form-item>
        <el-form-item label="提现备注:">
          <el-input type="textarea" style="width: 330px;height: 50px" :maxlength="128" v-model="formChange.orderRsv" placeholder="请输入提现备注"></el-input>
        </el-form-item>
        <el-form-item label="" style="display:flex;justify-content:center;">
          <div>
            <span style="color:red">
              <span>注:可提现账户余额&nbsp;</span>
              <span style="font-weight:bold">{{Number(userAmt).toFixed(2)}}</span>
              <span>（U）</span>
            </span>
          </div>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="userPush = false">取消</el-button>
        <el-button type="primary" style="background-color: #1ec695;" @click="submitForm()" :loading="tXLoading">确定
        </el-button>
      </div>
    </el-dialog> -->
  </div>
</template>

<script>
import API from '../../api'
import com from '../../common'

const formReq = {
  curcd: {
    isNoEmpty: '请输入币种！',
  },
  uNum: {
    isNoEmpty: '请输入币种数量',
    isNoNegative: '请输入正确的币种数量！',
  },
  uAddr: {
    isNoEmpty: '请输入币种地址！',
  },
}
export default {
  data() {
    return {
      UserId: '',
      userAmt: '',
      instAboutBtn: false,
      CurPage: 1,
      allPage: 1,
      // 查询
      formInline: {
        orderNo: '',
        tDate: [new Date(), new Date()],
      },
      instAboutList: {},
      tableData: [],
      //添加
      userPush: false,
      formChange: {
        curcd: 'USDT',
        uNum: '',
        uAddr: '',
        orderRsv: '',
      },
      tXLoading: false,
    }
  },
  created() {
    this.UserId = com.getLocalStorage('otc_webUserId')
  },
  mounted() {
    this.onSubmit()
  },
  methods: {
    _resetRorm() {
      //重置
      this.formChange = {
        curcd: 'USDT',
        uNum: '',
        uAddr: '',
        orderRsv: '',
      }
    },
    txClick() {
      //提现申请弹框
      this.infoUser()
      this.userPush = true
    },
    infoUser() {
      //用户信息
      let data = {
        userId: this.UserId,
      }
      API.phLogin('/infoUser.do', data).then((res) => {
        if (res.data.code == '0000') {
          this.userAmt = res.data.data.accAmt
        } else if (res.data.code == '9999') {
          this.$router.push('/login')
          this.$message.error('登录已失效,请重新登录!')
        } else {
          this.$message.error(res.data.msg)
        }
      })
    },
    submitForm() {
      //提现申请
      if (!com._checkParam(this.formChange, formReq)) {
        //表单规则
        return
      }
      let data = {
        userId: this.UserId,
        ...this.formChange,
      }
      this.tXLoading = true
      API.phLogin('/addTX.do', data).then(
        (res) => {
          this.tXLoading = false
          if (res.data.code == '0000') {
            this._resetRorm()
            this.$message.success('提现申请成功')
            this.onSubmit()
          } else if (res.data.code == '9999') {
            this.$router.push('/login')
            this.$message.error('登录已失效,请重新登录!')
          } else {
            this.$message.error(res.data.msg)
          }
        },
        (res) => {
          this.tXLoading = fasle
        }
      )
    },
    currentC(page) {
      this.CurPage = page
      this.clearTab()
      this.nextSubmit()
    },
    clearTab() {
      this.tableData = []
    },
    onSubmit() {
      // 点击查询按钮
      this.currentC(1)
    },
    nextSubmit() {
      // 查询数据
      let orderStatus = this.formInline.orderStatus || ''
      let orderNo = this.formInline.orderNo.toString()
      let tDate = this.formInline.tDate
      let bankAccNo = this.formInline.bankAccNo || ''
      if (tDate == null || tDate == undefined) {
        this.$message.error('请选择日期后提交')
        return
      }
      let data = {
        operId: this.UserId,
        orderNo,
        orderStatus,
        beginDate: API.format(tDate[0], 'yyyyMMdd'),
        endDate: API.format(tDate[1], 'yyyyMMdd'),
        curPage: this.CurPage.toString(),
      }
      API.phLogin('/txListG.do', data).then((res) => {
        if (res.data.code == '0000') {
          let data = res.data.data
          this.allPage = parseInt(data.totPage)
          if (data.totCnt == 0) {
            this.clearTab()
            return
          }
          this.tableData = data.reSult
        } else if (res.data.code == '9999') {
          this.$router.push('/login')
          this.$message.error('登录已失效,请重新登录!')
        } else {
          this.$message.error(res.data.msg)
        }
      })
    },
    userinfo(val2) {
      //详情
      let orderNo = val2
      this.instAboutBtn = true
      this.instAboutList = {}
      let data = {
        userId: this.UserId,
        orderNo: orderNo,
      }
      API.phLogin('/selTxnInfo.do', data).then((res) => {
        if (res.data.code == '0000') {
          this.instAboutList = res.data.data
        } else if (res.data.code == '9999') {
          this.$router.push('/login')
          this.$message.error('登录已失效,请重新登录!')
        } else {
          this.$message.error(res.data.msg)
        }
      })
    },
  },
}
</script>


<style scoped lang="stylus">
.userInfoQuery {
  border-radius: 10px;
  padding: 15px;
  background-color: #fff;
  box-sizing: border-box;

  .queryD {
    padding: 10px 22px 0 0;

    .demo-form-inline {
      margin-bottom: 22px;

      .el-form-item {
        margin-bottom: 0px;
      }
    }

    .el-form--inline {
      .el-form-item {
        margin-right: 5px;
      }
    }

    .demo-ruleForm1 {
      border: 1px solid rgba(1, 1, 1, 0.2);
      padding: 20px;
      width: 1042px;
      margin-bottom: 15px;

      .el-form-item {
        margin-bottom: 10px;
      }
    }
  }
}

.tableMsg {
  padding-top: 15px;
}

.userInfoQuery /deep/ .el-form-item__content {
  text-align: left;
}

.userInfoQuery /deep/ .instAbout .el-form-item__content {
  float: left;
  width: 230px !important;
}

.userInfoQuery /deep/ .push .el-form-item__content {
  float: left;
  margin-left: 10px !important;
}

.demo-table-expand .el-form-item {
  margin-right: 0;
  margin-bottom: 0;
  width: 49.5%;
}

.userInfoQuery /deep/ .instAbout .el-dialog {
  width: 800px;
}

.userInfoQuery /deep/ .push .el-dialog {
  width: 650px;
}

.userInfoQuery /deep/ .instAbout .demo-table-expand label {
  width: 130px !important;
  float: left;
  color: #99a9bf;
}

.userInfoQuery /deep/ .push label {
  float: left;
  color: #99a9bf;
}

.userInfoQuery /deep/ .el-dialog__close {
  color: white !important;
  font-size: 25px;
  margin: -5px -5px;
}

.userInfoQuery /deep/ .push .el-dialog__header {
  background-color: #1ec695;
  text-align: left;
  font-size: 15px;
  color: white;
}

.userInfoQuery >>> input::-webkit-outer-spin-button, .userInfoQuery >>> input::-webkit-inner-spin-button {
  -webkit-appearance: none;
}

.userInfoQuery >>> input[type='number'] {
  -moz-appearance: textfield;
}
</style>
