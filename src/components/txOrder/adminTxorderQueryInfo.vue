  ...
  <div class="userInfoQuery">
      ...
      <el-form :inline="true" :model="formInline" class="demo-form-inline">
        ...
        <el-form-item label="">
          <el-select size="small" v-model="formInline.orderStatus" placeholder="请选择" style="width: 180px">
            <el-option value="">默认</el-option>s
        ...
        <el-form-item label="">
          <el-input size="small" type="text" v-model="formInline.userId" placeholder="请输入商户ID" autocomplete="off" style="width: 180px;"></el-input>
        </el-form-item>
        <el-form-item>
    ...
    <div class="tableD">
      <el-table :data="tableData" :header-cell-style="{ background: 'rgba(250, 250, 250, 1)' }" style="width: 100%;font-size:13px;">
        ...
        <el-table-column prop="curcd" label="币种" width="80"></el-table-column>
        ...
        <el-table-column label="操作" width="230">
          <template slot-scope="scope">
            <el-button-group>
              <el-button icon="el-icon-info" size="small" style="background-color: #78909C;color:white;" @click="userinfo(scope.row)">详情</el-button>
              <el-button type="success" @click="notifyClick(scope.row)" icon="el-icon-check" size="small" :disabled="scope.row.orderStatus=='提现成功' || scope.row.orderStatus=='提现失败'">提现审核
              </el-button>
            </el-button-group>
          </template>
        </el-table-column>
    ...
    <el-dialog class="push" :visible.sync="checkDialog" width="550px">
      ...
      <el-form :model="checkForm" status-icon ref="ruleForm1" label-width="150px" class="demo-ruleForm1">
        ...
        <el-form-item label="订单数量:" style="text-align:left;">
          <div>{{checkForm.uNum}}&nbsp;U</div>
        </el-form-item>
        <el-form-item label="创建时间:" style="text-align:left;">
          <div>{{checkForm.crtTime}}</div>
        </el-form-item>
        <el-form-item label="状态:" style="text-align:left;margin-bottom:20px;">
          <el-radio v-model="checkForm.flag" label="提现成功" style="margin-top:14px;">提现成功</el-radio>
          <br>
          <el-radio v-model="checkForm.flag" label="提现失败" style="margin-top:14px;">提现失败</el-radio>
        </el-form-item>
        <el-form-item label="备注:" style="text-align:left;">
          <el-input type="textarea" style="width: 330px;height: 50px" :maxlength="128" v-model="checkForm.chkRsv" placeholder="请输入备注"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="checkDialog = false">取消</el-button>
        <el-button type="primary" style="background-color:#1ec695;" @click="_checkClick()">确定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    _checkClick() {
      ...
      API.phLogin("/chkTX.do", data).then((res) => {
    ...
    nextSubmit() {
      ...
      let data = {
        operId: this.UserId,
        orderNo: this.formInline.orderNo.toString(),
      ...
      API.phLogin("/txListS.do", data).then((res) => {
        if (res.data.code == "0000") {
...
.userInfoQuery /deep/ .push .el-form-item {
  margin-bottom: 0;
}
